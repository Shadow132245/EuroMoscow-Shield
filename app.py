...
JS_HEADER = f"/* Protected by EuroMoscow Shield V25 */\n"
LUA_HEADER = f"-- Protected by EuroMoscow Shield V25\n"

# ==========================================
# 1. PYTHON ENGINE (Encrypt & Decrypt)
# ==========================================
...
def dec_py(code):
    curr = code; pats = {'b64':r"base64\.b64decode\((['\"].*?['\"])\)", 'zlib':r"zlib\.decompress\((?:base64\.b64decode\((['\"].*?['\"])\)|(['\"].*?['\"]))\)", 'rot':r"codecs\.decode\((['\"].*?['\"]),\s*['\"]rot13['\"]\)", 'xor':r"c\^(\d+).*?in\s+(\[.*?\])", 'blob':r"zlib\.decompress\(bytes\(\[(.*?)\]\)\)"}
    def se(s): try: return ast.literal_eval(s); except: return s.strip("'").strip('"')
    for _ in range(25):
        dec=False; clean='\n'.join([l for l in curr.split('\n') if not l.strip().startswith('#')]).strip()
        m=re.search(pats['b64'],clean); 
        if m: try: curr=base64.b64decode(se(m.group(1))).decode(); dec=True; except: pass
        if not dec: m=re.search(pats['zlib'],clean); 
        if m: try: curr=zlib.decompress(base64.b64decode(se(m.group(1) or m.group(2)))).decode(); dec=True; except: pass
        if not dec: m=re.search(pats['rot'],clean); 
        if m: try: curr=codecs.decode(se(m.group(1)),'rot13'); dec=True; except: pass
        if not dec: m=re.search(pats['xor'],clean); 
        if m: try: curr=''.join(chr(c^int(m.group(1))) for c in eval(m.group(2))); dec=True; except: pass
        if not dec: m=re.search(pats['blob'],clean); 
        if m: try: curr=zlib.decompress(bytes(eval(f"[{m.group(1)}]"))).decode(); dec=True; except: pass
        if not dec: break
    return curr

# ==========================================
# 2. JS & LUA & PHP/HTML ENGINES
# ==========================================
...
def dec_js(code):
    curr = code; pats={'b64':r"eval\(atob\(['\"](.*?)['\"]\)\)", 'url':r"eval\(decodeURIComponent\(['\"](.*?)['\"]\)\)", 'hex':r"eval\(['\"](\\x[0-9a-fA-F]{2}.*?)['\"]\)", 'char':r"eval\(String\.fromCharCode\((.*?)\)\)"}
    for _ in range(25):
        dec=False; clean='\n'.join([l for l in curr.split('\n') if not l.strip().startswith('/*')]).strip()
        m=re.search(pats['b64'],clean);
        if m: try: curr=base64.b64decode(m.group(1)).decode(); dec=True; except: pass
        if not dec: m=re.search(pats['url'],clean);
        if m: try: curr=urllib.parse.unquote(m.group(1)); dec=True; except: pass
        if not dec: m=re.search(pats['hex'],clean);
        if m: try: curr=bytes.fromhex(m.group(1).replace('\\x','')).decode(); dec=True; except: pass
        if not dec: m=re.search(pats['char'],clean);
...
def dec_lua(code):
    curr = code; pats={'rev':r"string\.reverse\('((?:[^'\\]|\\.)*)'\)", 'hex':r'local _h="([0-9A-Fa-f]+)"', 'byte':r'local _b="([\d\s]+)"'}
    for _ in range(25):
        dec=False; clean=curr.replace('\n',' ').strip()
        m=re.search(pats['rev'],clean);
        if m: try: curr=m.group(1).replace("\\'","'").replace('\\"','"')[: : -1][::-1]; dec=True; except: 
            try: curr=m.group(1).replace("\\'","'").replace('\\"','"')[::-1]; dec=True; except: pass
        if not dec: m=re.search(pats['hex'],clean);
        if m: try: curr=bytes.fromhex(m.group(1)).decode('utf-8'); dec=True; except: pass
        if not dec: m=re.search(pats['byte'],clean);
...
def dec_php(code):
    ...
    if m: return zlib.decompress(base64.b64decode(m.group(1))).decode('utf-8')
    return code
...
# ==========================================
# 3. AI & TERMINAL & ROUTES
# ==========================================
def analyze_code_ai(code, lang):
    score = 100; risks = []
    if len(code)<20: return {"score":10, "msg":"Code too short."}
    if 'import' in code: risks.append("Libraries detected (Good).")
    if 'os.' in code or 'eval' in code: score-=30; risks.append("Unsafe functions found. Use 'Dead Code'.")
    return {"score":score, "msg":" | ".join(risks) if risks else "Clean Code."}

def chat_ai_bot(msg):
    msg = msg.lower()
    if "hello" in msg: return "Welcome to EuroMoscow Shield V25! 🛡️"
    if "analyze" in msg: return "Use the 'AI SCAN' button to check security."
    return "I am EuroMind. I secure code using polymorphic layers."

# EXECUTION ENGINE (FOR TERMINAL)
...
def process():
    try:
        d=request.json; c=d.get('code',''); a=d.get('action'); l=d.get('lang','python'); o=d.get('options',[]); e=d.get('expiry','')
        res=""
        if a=='encrypt':
    ...
    except Exception as x: return jsonify({'result':f"Error: {str(x)}"}),500

# Execution Route (For Terminal)
@app.route('/run', methods=['POST'])
def run():
    d = request.json; code = d.get('code', '')
    return jsonify({'output': execute_code_server(code)})

# AI Routes
@app.route('/analyze', methods=['POST'])
def analyze(): return jsonify(analyze_code_ai(request.json.get('code',''), request.json.get('lang','python')))

@app.route('/chat', methods=['POST'])
def chat(): return jsonify({'reply': chat_ai_bot(request.json.get('message',''))})
        ...
        with zipfile.ZipFile(m,'r') as zi, zipfile.ZipFile(out,'w',zipfile.ZIP_DEFLATED) as zo:
            for i in zi.infolist():
                ...
                try:
                    ...
                    elif i.filename.endswith('.lua'): zo.writestr(i.filename, proc_lua(d.decode('utf-8'),o))
                    else: zo.writestr(i,d)
                except: zo.writestr(i,d)
        out.seek(0); return send_file(out, mimetype='application/zip', as_attachment=True, download_name='Project_Encrypted.zip')
