...
def dec_js(code):
    ...
    for _ in range(25):
        dec=False; clean='\n'.join([l for l in curr.split('\n') if not l.strip().startswith('/*')]).strip()
        m=re.search(pats['b64'],clean);
        if m: try: curr=base64.b64decode(m.group(1)).decode(); dec=True
        except: pass
        if not dec: m=re.search(pats['url'],clean);
        if m: try: curr=urllib.parse.unquote(m.group(1)); dec=True
        except: pass
        if not dec: m=re.search(pats['hex'],clean);
        if m: try: curr=bytes.fromhex(m.group(1).replace('\\x','')).decode(); dec=True
        except: pass
        if not dec: m=re.search(pats['char'],clean);
        if m: try: curr="".join([chr(int(n)) for n in m.group(1).split(',')]); dec=True
        except: pass
        if not dec: break
    return curr
...
def dec_lua(code):
    ...
    for _ in range(25):
        ...
        if m: try: curr=m.group(1).replace("\\'","'").replace('\\"','"')[: : -1]; dec=True
        except: pass
        if not dec: m=re.search(pats['hex'],clean);
        if m: try: curr=bytes.fromhex(m.group(1)).decode('utf-8'); dec=True
        except: pass
        if not dec: m=re.search(pats['byte'],clean);
        if m: try: curr=bytes([int(x) for x in m.group(1).split()]).decode('utf-8'); dec=True
        except: pass
        if not dec: break
    return curr
...
def process():
    try:
        ...
        if a=='encrypt':
            ...
            if l=='javascript': res=smart_js_decrypt(c)
            elif l=='lua': res=smart_lua_decrypt(c)
            else: res=smart_py_decrypt(c)
...
def zip_up():
    try:
        f=request.files['file']; m=io.BytesIO(); o=request.form.get('options','').split(',')
        with zipfile.ZipFile(f,'r') as zi, zipfile.ZipFile(m,'w',zipfile.ZIP_DEFLATED) as zo:
            for i in zi.infolist():
                d=zi.read(i.filename)
                if i.filename.endswith('.py'): 
                    try: zo.writestr(i.filename, proc_py(d.decode(),o))
                    except: zo.writestr(i,d)
                elif i.filename.endswith('.js'):
                    try: zo.writestr(i.filename, proc_js(d.decode(),o))
                    except: zo.writestr(i,d)
                else: zo.writestr(i,d)
        m.seek(0); return send_file(m, mimetype='application/zip', as_attachment=True, download_name='Project_Encrypted.zip')
